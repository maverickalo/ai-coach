...
const exactDemoUrls: Record<string, string> = {};

const exactGifUrls: Record<string, string> = {};

const demoSearchUrls: Record<string, string> = {
...
export function exerciseDemoUrl(exerciseName: string): string {
  return exactDemoUrls[exerciseName] ?? demoSearchUrls[exerciseName] ?? youtubeSearchUrl(exerciseName);
}

export function exerciseGifUrl(exerciseName: string): string {
  return exactGifUrls[exerciseName] ?? gifSearchUrl(exerciseName);
}
...
export function exerciseResource(exerciseName: string): ExerciseResource {
  ...
  const demoIsExact = Boolean(exactDemoUrls[exerciseName]);
  const gifIsExact = Boolean(exactGifUrls[exerciseName]);
  return {
    demoUrl: exerciseDemoUrl(exerciseName),
    gifUrl: exerciseGifUrl(exerciseName),
    gifSearchUrl: gifSearchUrl(exerciseName),
    demoLabel: demoIsExact ? "video" : "video search",
    gifLabel: gifIsExact ? "GIF" : "image search",
    demoIsExact,
    gifIsExact,
    ...guide
  };
