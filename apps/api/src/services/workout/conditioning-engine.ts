...
export function recommendConditioning(
  ...
  if (recentPain) {
    return {
      ...
      sessionShape: "short",
      prescription:
        "Optional add-on: Assault Bike or rower intervals at conversational to moderate effort. Keep the strength plan unchanged and avoid hard running until pain details are clear.",
      reason:
  ...
  if (recentRunCount < 2) {
    return {
      ...
      prescription:
        "Optional add-on: add controlled treadmill or outdoor running after the main strength work, or use short easy run segments only if you explicitly want HYROX practice today.",
      reason:
  ...
  return {
    ...
    sessionShape: "hyrox_bias",
    prescription:
      "Optional add-on: keep the strength session as written, then add a HYROX-style finisher with moderate run, row, bike, sled, carries, wall balls, or slam balls.",
    reason:
